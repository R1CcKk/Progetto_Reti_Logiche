-- ============================================================
-- TESTBENCH CASO LIMITE 10: OP=11 (svuota) poi OP=01 (rimuovi)
-- Descrizione: Si inseriscono task, poi si svuota con OP=11,
--   poi si tenta OP=01 su lista vuota.
--   OP=11 deve: RAM[0]=0 (gli altri byte possono restare)
--   OP=01 dopo: deve restituire o_task_id=000000, RAM[0]=0.
-- Sequenza:
--   Step 0: insert ID=40 P=0
--   Step 1: insert ID=41 P=1
--   Step 2: OP=11 (clear) -> RAM[0]=0
--   Step 3: OP=01 (remove from empty) -> o_task_id=0, RAM[0]=0
-- Perche': Verifica che OP=11 aggiorni correttamente il
--   contatore e che il modulo non si confonda con i dati
--   residui in memoria dopo lo svuotamento.
-- ============================================================
...
architecture project_tb_arch of project_tb is
    ...
    signal scenario_config : scenario_config_type := (
        (task_id => "101000", task_priority => "00", op => "10"), -- Insert ID=40 P=0
        (task_id => "101001", task_priority => "01", op => "10"), -- Insert ID=41 P=1
        (task_id => "000000", task_priority => "00", op => "11"), -- OP=11: clear
        (task_id => "000000", task_priority => "00", op => "01")  -- OP=01: remove from empty
    );
    ...
    -- After clear we only check RAM[0]=0
    -- After remove-empty we only check RAM[0]=0
    constant CHECK_SIZE_ARRAY : int_array_t := (2, 3, 1, 1);

    signal scenario_result : scenario_result_type := (
        -- [count=1, ID=40|P=0=10100000]
        ("00000001", "10100000", others => "00000000"),
        -- [count=2, ID=40|P=0, ID=41|P=1=10100101]
        ("00000010", "10100000", "10100101", others => "00000000"),
        -- After OP=11: count=0
        ("00000000", others => "00000000"),
        -- After OP=01 on empty: count still 0
        ("00000000", others => "00000000")
    );

    -- For step 3, track expected o_task_id
    signal expected_task_id_step3 : std_logic_vector(5 downto 0) := "000000";
...
begin
    ...
    begin
        ...
        assert RAM(0) = "00000000"
            report "EDGE10 FALLITO: dopo reset RAM[0] != 0" severity failure;
        ...
        for i in 0 to SCENARIO_SIZE - 1 loop
            ...
            -- Step 3: OP=01 on empty list -> o_task_id must be 0
            if i = 3 then
                assert tb_o_task_id = "000000"
                    report "EDGE10 FALLITO @ STEP=3: o_task_id atteso=0, reale=" &
            ...
            for j in 0 to CHECK_SIZE_ARRAY(i) - 1 loop
                assert RAM(j) = scenario_result(i)(j)
                    report "EDGE10 FALLITO @ STEP=" & integer'image(i) &
            ...
            wait until falling_edge(tb_clk);
            report "EDGE10 Step " & integer'image(i) & " OK.";
        end loop;

        report "EDGE10 OK: OP=11 svuota correttamente, OP=01 su lista vuota restituisce 0";
        assert false report "Simulation Ended! EDGE10 PASSATO" severity failure;
    end process;
