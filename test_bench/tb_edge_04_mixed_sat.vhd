...
--   restano P=3. L'ordine relativo NON cambia (no riordino).
-- Memoria iniziale:
--   RAM[0]=3
--   RAM[1]=ID=5, P=2 -> 00010110
--   RAM[2]=ID=6, P=3 -> 00011011
--   RAM[3]=ID=7, P=3 -> 00011111
-- Dopo OP=00:
--   RAM[0]=3
--   RAM[1]=ID=5, P=3 -> 00010111  (era P=2, ora satura a P=3)
--   RAM[2]=ID=6, P=3 -> 00011011  (invariato)
--   RAM[3]=ID=7, P=3 -> 00011111  (invariato)
-- Perche': Verifica che la saturazione avvenga per-task e che
--   l'ordine originale sia preservato senza riordino.
-- ============================================================
...
architecture project_tb_arch of project_tb is
    ...
    -- 1: insert ID=6 P=3
    -- 2: insert ID=7 P=3
    -- 3: OP=00 -> P=2 saturates to P=3, P=3 stays
    ...
    signal scenario_config : scenario_config_type := (
        ...
        (task_id => "000000", task_priority => "00", op => "00")  -- OP=00
    );
    ...
    signal scenario_result : scenario_result_type := (
        -- After insert ID=5 P=2: count=1, [ID=5|P=2 = 00010110]
        ("00000001", "00010110", others => "00000000"),
        -- After insert ID=6 P=3: count=2, [ID=5|P=2, ID=6|P=3]
        ("00000010", "00010110", "00011011", others => "00000000"),
        -- After insert ID=7 P=3: count=3, [ID=5|P=2, ID=6|P=3, ID=7|P=3]
        ("00000011", "00010110", "00011011", "00011111", others => "00000000"),
        -- After OP=00: ID=5 P=2->3=00010111, ID=6 stays 00011011, ID=7 stays 00011111
...
begin
    ...
    begin
        ...
        assert RAM(0) = "00000000"
            report "EDGE4 FALLITO: dopo reset RAM[0] != 0" severity failure;
        ...
        for i in 0 to SCENARIO_SIZE - 1 loop
            ...
            for j in 0 to CHECK_SIZE_ARRAY(i) - 1 loop
                assert RAM(j) = scenario_result(i)(j)
                    report "EDGE4 FALLITO @ STEP=" & integer'image(i) &
            ...
            wait until falling_edge(tb_clk);
            report "EDGE4 Step " & integer'image(i) & " OK.";
        end loop;

        report "EDGE4 OK: OP=00 con mix P=2/P=3 satura correttamente senza riordino";
        assert false report "Simulation Ended! EDGE4 PASSATO" severity failure;
    end process;
